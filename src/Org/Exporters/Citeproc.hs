...
citeToCiteproc cite = C.Citation Nothing Nothing $
  foldMap refToItem (citationReferences cite)
  & changeFst (\c -> c { citationItemPrefix = liftA2 (<>)
                         (toCslJson <$> nonEmpty' (citationPrefix cite))
                         (citationItemPrefix c) })
  & changeLast (\c -> c { citationItemSuffix = liftA2 (<>)
  ...
  where
    ...
    refToItem ref = case getCiteStyle cite of
      Text -> [ (citeConstr AuthorOnly) { citationItemSuffix = Nothing }
      ...
      where
        citeConstr kind = CitationItem (ItemId $ refId ref)
